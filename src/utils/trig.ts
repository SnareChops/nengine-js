import type { Position, Box } from '../types/index.js'

export function directionVector(from: Position, to: Position): [number, number] {
...
/** Gets the distance between a position and the closest point on a box */
export function distanceToBounds(pos: Position, box: Box): number {
  const [mx, my] = box.min()
  const [Mx, My] = box.max()
  const px = Math.max(mx, Math.min(pos.x(), Mx))
  const py = Math.max(my, Math.min(pos.y(), My))
...
export function moveAway(x1: number, y1: number, x2: number, y2: number, speed: number, delta: number): [x: number, y: number] {
  const length = speed / delta
